import type {FfmpegExporterOptions} from '@reelgen/core';
...
export function getParamDefaultsAndCheckValidity(settings: RenderSettings): {
  ...
  if (
    settings.projectSettings?.exporter?.name === '@reelgen/core/image-sequence'
  ) {
    throw Error(
  ...
  if (
    settings.projectSettings?.exporter?.name === '@reelgen/core/wasm' &&
    extension !== 'mp4'
  ) {
    throw Error(
  ...
  if (settings.projectSettings?.exporter?.name === '@reelgen/core/wasm') {
    return {
      ...defaultReturn,
