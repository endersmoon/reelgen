...
import {backendFetch, backendWs} from './backend-client';
 ...
 * with the standalone backend server through a WebSocket connection which lets
 * it invoke methods on the FFmpegExporterServer class.
 *
...
export class FFmpegExporterClient implements Exporter {
  public static readonly id = '@reelgen/core/ffmpeg';
  ...
  private static initialize() {
    ...
    backendWs.on('revideo:ffmpeg-exporter-ack', (response: ServerResponse) => {
  ...
  public constructor(settings: RendererSettings) {
    ...
    FFmpegExporterClient.initialize();
  }
  ...
  public async stop(result: RendererResult): Promise<void> {
    ...
    await backendFetch('/revideo-ffmpeg-decoder/finished', {
      method: 'POST',
      headers: {
  ...
  public async generateAudio(
    ...
    await backendFetch('/audio-processing/generate-audio', {
      method: 'POST',
      body: JSON.stringify({
  ...
  public async mergeMedia(): Promise<void> {
    ...
    await backendFetch('/audio-processing/merge-media', {
      method: 'POST',
      body: JSON.stringify({
        outputFilename,
        tempDir,
  ...
  private invoke<TResponse = unknown, TData = unknown>(
    ...
    return new Promise((resolve, reject) => {
      ...
      backendWs.send('revideo:ffmpeg-exporter', {method, data});
    });
  }
}
